...
public class Bswabe {
	...
	private static String policy_to_string(BswabePolicy p) {
		 String out="";
//		System.out.println(p.k);
		if(p.children==null || p.children.length==0) {
//			System.out.println("k=1 "+p.attr);
			return p.attr;
		}else{
			for(int i=0; i<p.children.length; i++) {
				if(out.length()==0) {
					out+=policy_to_string(p.children[i]);
				}else {
					out+=" "+policy_to_string(p.children[i]);
				}
			}
			out+=" "+p.k +"of"+p.children.length;
		}
		return out;
	}
	
	public static String simplify_policy(String policy, String file_attr) throws Exception{
		
		BswabePolicy p = parsePolicyPostfix(policy);
		p.simplify(file_attr);
		return policy_to_string(p);
	}
}
